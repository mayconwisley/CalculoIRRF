...
public class InssServices(IInssRepository _inssRepository) : IInssServices
{
	...
	{
		return await _inssRepository.IsGov(vigencia);
	}

	public async Task<IEnumerable<InssGov>> ListarTodosPorVigencia(DateTime vigencia)
	{
		return await _inssRepository.GetByVigencia(vigencia);
	}
}
